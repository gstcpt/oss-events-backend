...
async function bootstrap() {
  ...
  // Swagger
  const config = new DocumentBuilder().setTitle('OSS Events API').setDescription('OSS Events API description').setVersion('2.3').addBearerAuth().build();
  const document = SwaggerModule.createDocument(app, config);
  SwaggerModule.setup('api/docs', app, document, { customCssUrl: 'https://unpkg.com/swagger-ui-dist@4.18.2/swagger-ui.css', customJs: ['https://unpkg.com/swagger-ui-dist@4.18.2/swagger-ui-bundle.js', 'https://unpkg.com/swagger-ui-dist@4.18.2/swagger-ui-standalone-preset.js'], });
...
export const handler: any = async (event: any, context: any, callback: any) => {
  ...
  if (context && typeof context.end === 'function') { return cachedApp(event, context); }
